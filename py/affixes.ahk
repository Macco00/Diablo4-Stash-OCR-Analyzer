affixes(fileName)
{
	...
	Loop 30
	{
		...
		if(InStr(rl, ")",, StrLen(rl) - 3) !=0)
		{
			rl := SubStr(rl, 1, (InStr(rl, "(",, 1)) - 1)
			if(rl = "" || rl = " ")
			{
				rl := nxt
				nxt := text.ReadLine()
		...
		if(InStr(nxt, ")",, StrLen(nxt) - 3) !=0)
		{
			nxt := SubStr(nxt, 1, (InStr(nxt, "(",, 1)) - 1)
			if(nxt = "" || nxt = " ")
				nxt := text.ReadLine()
		}
		...
		if (InStr(nxt, "%",, StrLen(nxt) - 3) != 0 || InStr(nxt, "]",, StrLen(nxt) - 3) != 0 || InStr(nxt, "1",, StrLen(nxt) - 3) !=0) ; if symbol exists
		{
			if(InStr(rl, "%",, StrLen(rl) - 3) = 0 || InStr(rl, "]",, StrLen(rl) - 3) = 0 || InStr(rl, "1",, StrLen(rl) - 3) = 0) ; if symbol does not exists
			{
		...
		else if (InStr(rl, "%",, StrLen(rl) - 3) != 0 || InStr(rl, "]",, StrLen(rl) - 3) != 0 || InStr(rl, "1",, StrLen(rl) - 3) != 0)
		{
	...
	while i <= affixes.Length
	{
		...
		res := StrReplace(res, "A", 4) ; if 4 is seen as an A
		res := StrReplace(res, ")", "") ; if ] is seen as an )
